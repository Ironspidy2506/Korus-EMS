...
export const getTravelExpenditureAttachment = async (travelExpenditureId: string) => {
  try {
    const token = localStorage.getItem('token');
    const response = await axios.get(`${API_BASE_URL}/attachment/${travelExpenditureId}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
  } catch (error) {
    console.error('Error fetching travel expenditure attachment:', error);
    throw error;
  }
}; 